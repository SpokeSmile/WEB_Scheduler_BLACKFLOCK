...
function App() {
  ...
  const [isLoadingUpdatesList, setIsLoadingUpdatesList] = reactExports.useState(false);
  const [isLoadingUpdateDetail, setIsLoadingUpdateDetail] = reactExports.useState(false);
  const [updatesError, setUpdatesError] = reactExports.useState("");
  const [selectedUpdateSlug, setSelectedUpdateSlug] = reactExports.useState(() => new URLSearchParams(window.location.search).get("patch") || "");
  ...
  function upsertSlot(slot) {
    setData((current) => ({
      ...current,
      slots: current.slots.some((existing) => existing.id === slot.id) ? current.slots.map((existing) => existing.id === slot.id ? slot : existing) : [...current.slots, slot]
    }));
    setSlotModal(null);
  }
  function removeSlot(id2) {
    setData((current) => ({
      ...current,
      slots: current.slots.filter((slot) => slot.id !== id2)
    }));
    setSlotModal(null);
  }
